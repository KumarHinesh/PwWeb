package com.vpt.pw.demo.model.FormCrf1;

import org.hibernate.annotations.GenericGenerator;

import javax.persistence.*;

@Entity
@Table(name = "ULTRASOUND_EXAMINATION")
public class UltrasoundExamination {
    @Id
    @GeneratedValue(
            strategy = GenerationType.IDENTITY,
            generator = "native"
    )
    @GenericGenerator(
            name = "native",
            strategy = "native"
    )
    @Column(name = "examination_id", nullable = false)
    private Integer id = 0;

    @ManyToOne(cascade = CascadeType.PERSIST)
    @JoinColumn(name = "form_crf1_id")
    private FormCrf1 formCrf1;

    @Column(name = "pw_crf_1_20")
    private String q20; //pw_crf_1_20

    @Column(name = "pw_crf_1_21")
    private String q21; //pw_crf_1_21

    @Column(name = "pw_crf_1_22")
    private String q22; //pw_crf_1_22

    @Column(name = "pw_crf_1_23")
    private String q23; //pw_crf_1_23

    @Column(name = "pw_crf_1_24")
    private String q24; //pw_crf_1_24

    @Column(name = "pw_crf_1_25")
    private String q25; //pw_crf_1_25

    @Column(name = "pw_crf_1_26")
    private String q26; //pw_crf_1_26

    @Column(name = "pw_crf_1_27")
    private String q27;//pw_crf_1_27

    @Column(name = "pw_crf_1_28")
    private String q28;//pw_crf_1_28

    @Column(name = "pw_crf_1_29")
    private String q29;//pw_crf_1_29

    @Column(name = "pw_crf_1_30")
    private String q30;//pw_crf_1_30

    @Column(name = "pw_crf_1_31")
    private String q31;//pw_crf_1_31

    @Column(name = "pw_crf_1_32")
    private String q32;//pw_crf_1_32

    @Column(name = "pw_crf_1_33")
    private String q33;//pw_crf_1_33

    @Column(name = "pw_crf_1_34")
    private String q34;//pw_crf_1_34

    @Column(name = "pw_crf_1_35")
    private String q35;//pw_crf_1_35

    @Column(name = "pw_crf_1_36")
    private String q36;//pw_crf_1_36

    @Column(name = "pw_crf_1_37")
    private String q37;//pw_crf_1_37

    public Integer getId() {
        return id;
    }

    public void setId(Integer id) {
        this.id = id;
    }

    public String getQ20() {
        return q20;
    }

    public void setQ20(String q20) {
        this.q20 = q20;
    }

    public String getQ21() {
        return q21;
    }

    public void setQ21(String q21) {
        this.q21 = q21;
    }

    public String getQ22() {
        return q22;
    }

    public void setQ22(String q22) {
        this.q22 = q22;
    }

    public String getQ23() {
        return q23;
    }

    public void setQ23(String q23) {
        this.q23 = q23;
    }

    public String getQ24() {
        return q24;
    }

    public void setQ24(String q24) {
        this.q24 = q24;
    }

    public String getQ25() {
        return q25;
    }

    public void setQ25(String q25) {
        this.q25 = q25;
    }

    public String getQ26() {
        return q26;
    }

    public void setQ26(String q26) {
        this.q26 = q26;
    }

    public String getQ27() {
        return q27;
    }

    public void setQ27(String q27) {
        this.q27 = q27;
    }

    public String getQ28() {
        return q28;
    }

    public void setQ28(String q28) {
        this.q28 = q28;
    }

    public String getQ29() {
        return q29;
    }

    public void setQ29(String q29) {
        this.q29 = q29;
    }

    public String getQ30() {
        return q30;
    }

    public void setQ30(String q30) {
        this.q30 = q30;
    }

    public String getQ31() {
        return q31;
    }

    public void setQ31(String q31) {
        this.q31 = q31;
    }

    public String getQ32() {
        return q32;
    }

    public void setQ32(String q32) {
        this.q32 = q32;
    }

    public String getQ33() {
        return q33;
    }

    public void setQ33(String q33) {
        this.q33 = q33;
    }

    public String getQ34() {
        return q34;
    }

    public void setQ34(String q34) {
        this.q34 = q34;
    }

    public String getQ35() {
        return q35;
    }

    public void setQ35(String q35) {
        this.q35 = q35;
    }

    public String getQ36() {
        return q36;
    }

    public void setQ36(String q36) {
        this.q36 = q36;
    }

    public String getQ37() {
        return q37;
    }

    public void setQ37(String q37) {
        this.q37 = q37;
    }

    public FormCrf1 getFormCrf1() {
        return formCrf1;
    }

    public void setFormCrf1(FormCrf1 formCrf1) {
        this.formCrf1 = formCrf1;
    }
}
